...
?>
    ...
    foreach ($daftar_user as $key) {
        ...
        for ($x = 1; $x < $tanggal + 1; $x++) {
            ...
            // kolom jam masuk
            echo '<td class="time col-masuk ' . tgl_indomerah($tahun, $bulan, $x) .' '. genab_ganjil($x). '">';
            ...
            echo '</td>';
            //kolom jam pulang
            echo '<td class="time col-pulang ' . tgl_indomerah($tahun, $bulan, $x) .' '. genab_ganjil($x).'">'; 
                if(strtotime($RekapHariTerakhir) > strtotime($tgl) AND ($whari != 'Sabtu' AND $whari != 'Minggu')){
                    echo $wPulang = $this->Rekap_model->getRekap_time($idAdminInstansi, $key['id_user'], $tgl, 'out'); 
